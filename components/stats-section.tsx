...
export function StatsSection() {
  ...
  const students = useCountUp(145, 2500, "K+")
  const campuses = useCountUp(4, 1500, "")
  const confidentiality = useCountUp(100, 2000, "%")
    ...
    <section id="stats-section" className="py-24 px-6 bg-background">
      <div className="max-w-4xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 lg:gap-16">
          ...
          >
            <p className="font-light text-foreground mb-2 text-6xl md:text-7xl leading-none">{students.value}</p>
            <p className="text-xs text-muted-foreground uppercase tracking-wider">Sun Devils</p>
          </div>

          <div
            className={`text-center transition-all duration-1000 delay-300 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
          >
            <p className="font-light text-foreground mb-2 text-6xl md:text-7xl leading-none">{campuses.value}</p>
            <p className="text-xs text-muted-foreground uppercase tracking-wider">Major Campuses</p>
          </div>

          <div
            className={`text-center transition-all duration-1000 delay-400 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
          >
            <p className="font-light text-foreground mb-2 text-6xl md:text-7xl leading-none">{confidentiality.value}</p>
